...
|---|---|---|
| 围绕资料学习 | NotebookLM | 绑定 Gemini + Google 云，输出孤立、无法流入后续练习。DeepStudent 本地存储、多模型可选，同一份 PDF 连续完成：问答 → 导图 → 出题 → 制卡 → Anki，每步输出即下步输入 |
| 笔记与知识库 | Notion / Obsidian | 通用笔记工具，AI 后期外挂。DeepStudent 导入即触发 OCR → 向量化 → 索引，全部内容 AI 可检索；笔记、教材、题库、导图、翻译共享统一 VFS |
| 思维整理 | XMind / 幕布 | 需手动绘制。DeepStudent 从学习材料出发，一句话生成完整知识结构，支持多轮对话编辑 + 节点遮挡背诵 |
| 刷题与复习 | Quizlet / Anki | 题目需手动录入，Anki 制卡繁琐。DeepStudent：上传试卷 → AI 出题 → 每日 / 限时 / 模考 → 知识点掌握率追踪 → 一键导出 Anki |
| 翻译精读 | DeepL | 翻译完即结束，与学习流程脱节。DeepStudent 翻译留在工作台——逐段双语对照，5 种领域预设，翻译完可直接提问、出题、制卡 |

> **架构上的核心区别：** 统一虚拟文件系统（VFS）让所有学习数据对 AI 可读、可搜索、可写入。聊天 Agent 可以检索教材和笔记来回答问题，也可以直接生成导图、题目、闪卡、调研报告并写回系统。一份材料在同一个工作台里完成全链路，无需在多个软件之间搬运数据。
